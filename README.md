...
- **2026-07-13** — Kill re-share-of-quiet-page bug + hard-block Kościół Uliczny.
  - **Incident**: bgnews wall got a wave of Kościół Uliczny Boguszów-Gorce photos dated 2021-2022 on 2026-07-13. Root cause: the page hadn't posted anything new in months, so every scrape returned the same old photos. `shared_posts.json` was pruning entries after `DAYS_TO_KEEP = 7` — so the same fbid could re-appear as "unseen" 8 days after previous share. Cycle: share Jun 29 → prune Jul 6 → re-share Jul 13.
  - **Fix 1**: removed Kościół Uliczny (`profile.php?id=100067837419514`) from `MONITORED_PAGES` per user request. Comment explains why to prevent future re-additions.
  - **Fix 2**: new `BLOCKED_SOURCE_URLS` set (defense-in-depth). Any source URL containing a blocked token is silently skipped at both `scrape_all_monitored_pages` (unauth) and `scrape_all_auth_pages` (Selenium) entry points, even if it gets re-added to `MONITORED_PAGES` by accident. Kościół Uliczny profile id `100067837419514` is the initial entry.
  - **Fix 3**: bumped `DAYS_TO_KEEP` 7 → 365. One year of memory for shared URLs covers any quiet-page cycle. State file grows ~1 KB/URL so a year of daily shares = ~10 MB — trivial.
- **2026-06-30** — Repair full share pipeline (mirror of wch same-day repair). Three orthogonal bugs uncovered live:
  - `verify_share_succeeded()` — after clicking "Udostępnij w Aktualnościach" + composer Dalej, confirm the post is on our own wall by DOM-needle match. Previously the code assumed success after 30s timeout waiting for a confirmation toast that FB shows only intermittently → 645/655 "successful" shares were silent fails. Post-fix, fail-closed if verify can't run.
  - `_find_share_button_for_post()` + `_click_robust()` + `_strip_pl()` — proper share button targets `<div role='button' aria-label='Udostępnij post <Page Name>'>` (not the legacy span). Diacritic-insensitive substring match against source page name, JS click fallback on interception.
  - `share_post()` composer flow — after "Udostępnij w Aktualnościach", FB opens a "Napisz post" composer that requires a `Dalej` click, then FB switches the dialog title to "Ustawienia posta" (Post Settings), where the FINAL blue `Udostępnij` submit lives in a sibling DOM portal outside the dialog scope. Both clicks were missing → composer stayed open forever, share never landed. Fixed with polling + aria-label direct match at document scope.
- **2026-06-30** — Add `MAX_ATTEMPTS_PER_WARNING_ID` (=2) safety cap in `bg_weather_map_selenium.py` + `bg_rcb_alert_checker.py`. If verify falsely reports failure and cron republishes hourly, the cap breaks the loop at 2 attempts (14× spam incident 2026-06-29 no longer possible). Atomic `_save_posted_alerts` via `os.replace` closes the residual race window.
- **2026-06-30** — Bumped `MAX_POSTS_PER_SOURCE` 3 → 7. With a cap of 3, on days when a prolific source (e.g. Burmistrz) posted 2-3 things in one burst, older posts from earlier that day permanently fell off the top-3 window before cron caught them. Concrete miss: Burmistrz's post about 8th-graders visiting his office (fbid 122114608305) — visible at posinset=7 on Jun 30 but never harvested because we only ever kept posinsets 4-6.
